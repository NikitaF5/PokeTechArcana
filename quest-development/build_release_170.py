...
previous = json.loads((QUEST / "release-1.6.0" / "manifest.json").read_text("utf-8"))
previous["pack_version"] = "1.7.0"
previous["config_zip"] = {
    "url": "https://github.com/NikitaF5/PokeTechArcana/releases/download/pack-v1.7.0/configs.zip",
    "sha1": sha1(configs),
}
(OUT / "manifest.json").write_text(json.dumps(previous, ensure_ascii=False, indent=2), "utf-8")
print(f"configs.zip {configs.stat().st_size} {sha1(configs)}")
print(f"manifest files {len(previous['files'])}")
